...
export const GuestSeat: React.FC<GuestSeatProps> = ({ guest, isActive, onToastClick, onKickClick, tableRadius }) => {
  ...
  return (
    <div
      // Outer container handles POSITION (x, y) and centering translate.
      // This prevents the inline style transform from overwriting the rotation class.
      className={`absolute flex flex-col items-center justify-center transition-all ease-in
         ${isKicking ? 'duration-700' : 'duration-300'}
      `}
      style={{
        left: `calc(50% + ${x}px)`,
        top: `calc(50% + ${y}px)`,
    ...
    >
      {/* Inner container handles ROTATION and SCALE animation */}
